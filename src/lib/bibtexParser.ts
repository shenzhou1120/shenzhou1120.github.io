...
function parseBibTeXEntries(input: string): RawBibTeXEntry[] {
  ...
  while (cursor < input.length) {
    ...
    const body = input.slice(bodyStart, end - 1);
    ...
    const citationKey = body.slice(0, comma).trim();
    const fields = body.slice(comma + 1);
    const entryTags: Record<string, string> = {};
    let fieldCursor = 0;

    while (fieldCursor < fields.length) {
      while (fieldCursor < fields.length && /[\s,]/.test(fields[fieldCursor])) fieldCursor += 1;
      const keyMatch = fields.slice(fieldCursor).match(/^([a-zA-Z][\w-]*)\s*=\s*/);
      if (!keyMatch) break;

      const key = keyMatch[1].toLowerCase();
      fieldCursor += keyMatch[0].length;
      let value = '';

      if (fields[fieldCursor] === '{') {
        fieldCursor += 1;
        let valueDepth = 1;
        const valueStart = fieldCursor;
        while (fieldCursor < fields.length && valueDepth > 0) {
          if (fields[fieldCursor] === '{') valueDepth += 1;
          if (fields[fieldCursor] === '}') valueDepth -= 1;
          fieldCursor += 1;
        }
        value = fields.slice(valueStart, fieldCursor - 1);
      } else if (fields[fieldCursor] === '"') {
        fieldCursor += 1;
        const valueStart = fieldCursor;
        while (fieldCursor < fields.length && fields[fieldCursor] !== '"') fieldCursor += 1;
        value = fields.slice(valueStart, fieldCursor);
        fieldCursor += 1;
      } else {
        const valueStart = fieldCursor;
        while (fieldCursor < fields.length && fields[fieldCursor] !== ',') fieldCursor += 1;
        value = fields.slice(valueStart, fieldCursor).trim();
      }

      entryTags[key] = value.trim();
    }
